...
import redis_task_list as task_list
...
def test_get_tasks():
    task_list.save_task({'description' : "This is a test meeting.", 'status' : "1"})
    task_list.save_task({'description' : "This is another test meeting.", 'status' : "1"})
    tasks = task_list.get_tasks()
    assert type(tasks) is list
    for task in tasks:
        for item in ['_id','description','status']:
            assert type(task[item]) is str
        assert task['status'] in "01"

def test_get_tasks_by_status():
    task_list.save_task({'description' : "This is an inactive meeting.", 'status' : "0"})
    task_list.save_task({'description' : "This is an active meeting.", 'status' : "1"})
...
def test_get_task():
    ...
    task_description = task_list.get_tasks()[0]['description']
    task_status = task_list.get_tasks()[0]['status']
